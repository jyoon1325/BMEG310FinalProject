...
# Use the filtered mutation matrix
# Assume `gene_patient_matrix` is the mutation data with patients as columns and genes as rows
# Ensure columns are patients, rows are genes, and all data is numeric (0/1)

# Prepare mutation count data (top 20 genes or full dataset)
mutation_counts <- as.matrix(gene_patient_matrix[, -c(1, ncol(gene_patient_matrix))])  # Exclude Hugo_Symbol and Mutation_Counts
rownames(mutation_counts) <- gene_patient_matrix$Hugo_Symbol  # Set rownames as gene symbols

# Ensure patient IDs are consistent between clustering and mutation matrix
valid_patients <- intersect(colnames(mutation_counts), names(cluster_assignments))

# Subset mutation counts matrix and cluster assignments
mutation_counts <- mutation_counts[, valid_patients, drop = FALSE]
cluster_labels <- cluster_assignments[valid_patients]

# Create DESeq2 dataset
dds <- DESeqDataSetFromMatrix(
  countData = mutation_counts,
  colData = data.frame(cluster = as.factor(cluster_labels)),
  design = ~ cluster
)

# Run DESeq2 analysis
dds <- DESeq(dds)
res <- results(dds, alpha = 0.05)  # Adjusted p-value threshold of 0.05

# Extract significantly differentially mutated genes
